import { addUser, deleteUser, getUsers, updateUser } from '@/app/api/users/userService';
...
describe('userService', () => {
    ...
    describe('updateUser', () => {
        it('should update user data when provided valid input', async () => {
            ...
            (prisma.user.update as jest.Mock).mockResolvedValue({ id: 1, name: 'Updated User' }); // User update successful
        ...
        it('should handle error when user update fails', async () => {
            // Mock Prisma client behavior for testing
            (prisma.user.findFirst as jest.Mock).mockResolvedValue(null); // No existing user found
            (prisma.user.update as jest.Mock).mockRejectedValue(new Error('Failed to update user!')); // User update fails
        ...
        it('should handle error when email is already in use', async () => {
            // Mock Prisma client behavior for testing
            (prisma.user.findFirst as jest.Mock).mockResolvedValue({ id: 2, email: 'existing@example.com' }); // Existing user found

            // Test updateUser function when email is already in use
            const updatedUser = await updateUser(1, { email: 'existing@example.com' });
    ...
    describe('addUser', () => {
        it('should create a new user when provided valid input', async () => {
            // Mock Prisma client behavior for testing
            (prisma.user.findFirst as jest.Mock).mockResolvedValue(null); // No existing user found
            (prisma.user.create as jest.Mock).mockResolvedValue({ id: 1, name: 'New User' }); // User creation successful

            // Test addUser function with valid input
            const newUser = await addUser({ name: 'New User', email: 'newuser@example.com' });
        ...
        it('should handle error when email is already in use', async () => {
            ...
            const newUser = await addUser({ name: 'New User', email: 'existing@example.com' });

            expect(newUser.error).toBeTruthy(); // Error flag should be true
            expect(newUser.message.email).toBe('Email already in use by another user!'); // Error message should indicate email conflict
        });

        it('should handle error when user creation fails', async () => {
            // Mock Prisma client behavior for testing
            (prisma.user.findFirst as jest.Mock).mockResolvedValue(null); // No existing user found
            (prisma.user.create as jest.Mock).mockRejectedValue(new Error('Failed to create user')); // User creation fails
